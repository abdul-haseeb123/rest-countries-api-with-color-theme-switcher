...
export const getData = () => {
    const countries = mydata.map((c) => {
        return { name: c.name, population: c.population, region: c.region, flags: c.flags, capital: c.capital }
    })
...
export const getDataByregionAndName = (region, name) => {
    const data = getData()
    const filteredcountries = data.filter((c) => {
        if ((c.region.toLowerCase() === region.toLowerCase()) && (c.name.toLowerCase().startsWith(name))) {
            return c
        }
    })
    ...
    const country = mydata.map((c) => {
        if (c.name.toLowerCase() === name.toLowerCase()) {
            ...
            return result = {
                ...
                languages: c.languages, capital: c.capital, borderCountries: borderCountries,
                flags:c.flags
            }
        }
